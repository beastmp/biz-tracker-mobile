...
export default function NewPurchase() {
  ...
  // Recalculate totals when items, tax rate, or shipping cost change
  useEffect(() => {
    ...
    const total = subtotal + taxAmount + (formData.shippingCost || 0);
  ...
  useEffect(() => {
    if (selectedItem) {
      ...
      if (selectedItem.trackingType === 'weight') {
        setTotalCost((weightNum * costNum).toFixed(2));
      } else {
        setTotalCost((qtyNum * costNum).toFixed(2));
      }
    }
  }, [selectedItem, quantity, weight, costPerUnit]);
  ...
  const handleSelectItem = (item: Item) => {
    ...
    setCostPerUnit(item.price.toString());
    setQuantity('1');
    setWeight('0');
  ...
  const handleAddItem = () => {
    ...
    let itemQty = 1;
    let itemWeight = 0;
    let itemCost = parseFloat(costPerUnit) || 0;
    let itemTotalCost = parseFloat(totalCost) || 0;
    ...
    if (itemCost <= 0) {
      setError('Cost per unit must be greater than zero');
      return;
    }
  ...
  const validateForm = (): string | null => {
    ...
    if ((formData.total || 0) <= 0) {
      return 'Purchase total must be greater than zero';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await purchasesApi.create(formData as Purchase);
      Alert.alert('Success', 'Purchase was created successfully');
              ...
              {selectedItem.trackingType === 'weight' ? (
                <View style={styles.quantityContainer}>
                  ...
                  <Picker
                    ...
                    onValueChange={(itemValue) => setWeightUnit(itemValue as 'oz' | 'lb' | 'g' | 'kg')}
